...
BEGIN TRAN
USE [One.Components]
DECLARE @PropRef nvarchar(50) = 'FAIRFA00%'
...
USE [One.Components]
DECLARE @PropertyRef nvarchar(50) = 'FAIRFA00%'
Declare @DefaultSupplier INT = 187 --@DEFAULT SUPPLIER@

IF (select count(*) from Component where PropertyReference in (select PropertyRef from Property where PropertyRef like @PropertyRef))>0
BEGIN
...
insert into @Property select replace(propertyRef, ' ', ''), levelCode from property where PropertyRef like @PropertyRef order by PropertyRef
...
DECLARE @CompID INT
SET @CompID = (SELECT MIN(Id) from Component where PropertyReference in (select PropertyRef from Property where PropertyRef like @PropertyRef))
