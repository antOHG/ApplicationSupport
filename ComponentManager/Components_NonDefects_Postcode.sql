...
1. UPDATE the @Postcode with an appropriate reference, say 'N6 4EY%'

2. Execute the 'MAIN SCRIPT', if the result shows several successful messages - then the activity is completed

3. If the result shows "Components already exists", then DELETE the components by running the following with appropriate @Postcode:

BEGIN TRAN
USE [One.Components]
DECLARE @Postcode nvarchar(50) = 'N6 4EY%'
...
4. If the above is successful, execute the script with COMMIT action
...
(select Id from Component where PropertyReference in (select PropertyRef from Property where Postcode like @Postcode) AND ComponentType_Id IN (13,15,18))
